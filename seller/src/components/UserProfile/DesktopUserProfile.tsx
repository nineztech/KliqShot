...
import { FileText, Download, User,MessageCircle, Calendar, FileSpreadsheet, Edit2 } from 'lucide-react';
...
const UserProfile: React.FC = () => {
 const { isMinimized } = useSidebar();
  ...
  const teamMembers = [
  { id: 1, name: "Aarav Mehta", role: "Lead Photographer" },
  { id: 2, name: "Priya Sharma", role: "Cinematographer" },
  { id: 3, name: "Rohan Patel", role: "Photo Editor" },
  { id: 4, name: "Isha Verma", role: "Creative Director"},
];
  ...
  <button
    ...
    className={`w-full py-2.5 px-4 rounded-xl text-left transition-all duration-200 flex items-center ${
      activeTab === 'personal'
        ...
        : 'bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 text-gray-800 hover:from-indigo-100 hover:to-pink-100'
    }`}
  >
    <User className={`w-4 h-4 mr-3 ${activeTab === 'personal' ? 'text-white' : 'text-indigo-600'}`} />
  ...
  >
    <Calendar className={`w-4 h-4 mr-3 ${activeTab === 'appointments' ? 'text-white' : 'text-indigo-600'}`} />
    Appointments
  </button>
  ...
  >
    <FileSpreadsheet className={`w-4 h-4 mr-3 ${activeTab === 'records' ? 'text-white' : 'text-indigo-600'}`} />
    Team Records
  </button>
</div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6 border border-gray-100 opacity-0 animate-fadeInUp" style={{ animationDelay: '0.2s' }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 md:gap-x-8 gap-y-4 md:gap-y-5">
                <div className="group">
                  ...
                  <p className="text-sm md:text-base font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">July 28, 2000</p>
                </div>
                <div className="group">
                  <p className="text-xs md:text-sm text-gray-500 mb-1">Gender</p>
                  <p className="text-sm md:text-base font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">Female</p>
                </div>
                <div className="group">
                  <p className="text-xs md:text-sm text-gray-500 mb-1">Phone Number</p>
                  <p className="text-sm md:text-base font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">+123 456 7890</p>
                </div>
                <div className="group">
                  <p className="text-xs md:text-sm text-gray-500 mb-1">Blood Type</p>
                  <p className="text-sm md:text-base font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">O+</p>
                </div>
                <div className="group">
                  <p className="text-xs md:text-sm text-gray-500 mb-1">Address</p>
                  <p className="text-sm md:text-base font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">Srengseng, Kec Kb, Jakarta</p>
                </div>
                <div className="group">
                  <p className="text-xs md:text-sm text-gray-500 mb-1">Insurance Provider</p>
                  <p className="text-sm md:text-base font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">2365846</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6 border border-gray-100 opacity-0 animate-fadeInUp" style={{ animationDelay: '0.3s' }}>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-3">
                ...
                <button className="text-indigo-600 hover:text-indigo-700 text-md font-medium hover:underline self-start sm:self-auto">
                  View All
                </button>
              </div>
              ...
              <div className="space-y-3 md:space-y-4">
                {appointments.map((apt, idx) => (
                  <div 
                    key={idx} 
                    className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 md:p-4 border-2 border-gray-200 rounded-xl hover:border-indigo-300 hover:shadow-md transition-all duration-200 gap-3 opacity-0 animate-fadeInLeft"
                    style={{ animationDelay: `${0.4 + idx * 0.1}s` }}
                  >
                    <div className="flex items-center gap-3 md:gap-4 flex-1">
                      <div className="text-center bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-2 md:p-3 min-w-[60px] md:min-w-[70px]">
                    ...
                    <button className="px-4 py-2 bg-gradient-to-r from-slate-800 via-purple-800 to-indigo-900 text-white text-xs md:text-sm rounded-lg hover:from-slate-700 hover:to-indigo-800 hover:scale-105 transition-all duration-200 shadow-md hover:shadow-lg font-medium self-start sm:self-auto">
            ...
            {/* Files Section */}
          <div
      className="bg-white rounded-2xl shadow-lg p-4 md:p-6 border border-gray-100 opacity-0 animate-fadeInUp"
      style={{ animationDelay: "0.5s" }}
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-3">
        <h3 className="text-lg md:text-xl font-bold text-gray-900">Team</h3>
        <button className="text-indigo-600 hover:text-indigo-700 text-sm font-medium hover:underline self-start sm:self-auto">
          View All
        </button>
      </div>
        ...
        {teamMembers.map((member, idx) => (
          ...
          >
            ...
            <button className="p-2 hover:bg-red-100 rounded-lg transition-colors flex-shrink-0 ml-2">
              <MessageCircle className="w-4 h-4 md:w-5 md:h-5 text-gray-500 hover:text-red-500" />
            </button>
          </div>
        ))}
      </div>
